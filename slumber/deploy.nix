...

{
  ...
  home.packages = [
    (pkgs.writeShellScriptBin "slumber-deploy" ''
      ...
      ssh slumber@$(cat ${config.sops.secrets."tailscale/ipv4s/slumber".path}) "sed -i 's/MAC_ADDRESS/$MAC_ADDRESS/g' ~/poweredge-bot.py; sed -i 's/BOT_KEY/$BOT_KEY/g' ~/poweredge-bot.py; systemctl --user daemon-reload; systemctl --user enable poweredge-bot.service; (systemctl --user restart poweredge-bot.service || systemctl --user start poweredge-bot.service)"
    '')
  ];
}
